package com.imss.sivimss.dispoSalas.beans;

import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.imss.sivimss.dispoSalas.model.ReporteDto;
import com.imss.sivimss.dispoSalas.model.UsuarioDto;
import com.imss.sivimss.dispoSalas.model.request.RegistrarEntradaSalaModel;
import com.imss.sivimss.dispoSalas.util.AppConstantes;
import com.imss.sivimss.dispoSalas.util.DatosRequest;
import com.imss.sivimss.dispoSalas.util.QueryHelper;
...
public class Salas {
    public DatosRequest buscarSalas(DatosRequest request) {
        ...
        String query = "SELECT " +
                ...
                "AND DATE_FORMAT(BS.TIM_HORA_ENTRADA, '%H:%i:%s' ) <= DATE_FORMAT(NOW( ), '%H:%i:%s' ) " +
                "AND IFNULL(BS.TIM_HORA_SALIDA, DATE_FORMAT(NOW( ), '%H:%i:%s' )) >= DATE_FORMAT(NOW( ), '%H:%i:%s' ) " +
                "THEN 'MANTENIMIENTO' " +
                "WHEN BS.FEC_ENTRADA = CURDATE() " +
    ...
    public DatosRequest consultarPorMes(DatosRequest request) {
        ...
        String query = "SELECT " +
                ...
                "      AND DATE_FORMAT(SBS.TIM_HORA_ENTRADA, '%H:%i:%s' ) <= DATE_FORMAT(NOW( ), '%H:%i:%s' ) " +
                "      AND IFNULL(SBS.TIM_HORA_SALIDA, DATE_FORMAT(NOW( ), '%H:%i:%s' )) >= DATE_FORMAT(NOW( ), '%H:%i:%s' ) THEN 'MANTENIMIENTO' " +
                "      WHEN SBS.FEC_ENTRADA = CURDATE() " +
                "      AND SBS.ID_TIPO_OCUPACION = 2 " +
                "      AND DATE_FORMAT(SBS.TIM_HORA_ENTRADA, '%H:%i:%s' ) <= DATE_FORMAT(NOW( ), '%H:%i:%s' ) " +
                "      AND IFNULL(SBS.TIM_HORA_SALIDA, DATE_FORMAT(NOW( ), '%H:%i:%s' )) >= DATE_FORMAT(NOW( ), '%H:%i:%s' ) THEN 'OCUPADA' " +
                "      ELSE 'DISPONIBLE' " +
                "   END estadoSala, " +
